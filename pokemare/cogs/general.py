...
"""
        embed = Embed(description=desc, color=ctx.bot.color).set_author(
            name="PokéMare"
        )  # .set_image(url='https://media.discordapp.net/attachments/936820715938795561/937542712473812992/image0.gif')
        await ctx.reply(embed=embed)

    @command(name="ping", description="Latency of the PokeMare")
    ...
    async def _invite(self, ctx: Context):
        """PokeMare invite"""
        await ctx.send(
            f"> The bot is still in a development stage! You would like to test it out so we have made the invite link available",
            embed=Embed(
                color=ctx.bot.color,
                description=f"> {ctx.bot.get_emoji(866957054278631444)} Invite PokeMare by [`clicking here`]({ctx.bot.invite_url})\n> ♥️ We have a [`support server`]({ctx.bot.support_server_invite_url}) too",
            ),
        )
