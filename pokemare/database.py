...
class UserInfoDatabase:
    ...
    async def insert_user_into_database(
        self, user_id: str, gender: str, starter_id: str
    ) -> None:
        async with self.user_database.cursor() as cursor:
            await cursor.execute(
                """
                INSERT INTO users
                ( user_id , gender , starter_id ,  pokedollars , stars )
                VALUES ( ? , ? , ? , ? , ?)
                """,
                (user_id, gender, starter_id, "2500", "0"),
            )
            await self.user_database.commit()
